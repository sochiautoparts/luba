"""Люба Group handler — active participation + web search + partners + site content."""
...
async def handle_group_photo(message):
    ...
    try:
        ...
        if data_uri:
            vision_prompt = f"Тебе прислали фото в группе. Опиши кратко что видишь (1-2 предложения), потом живо отреагируй как Люба. {'Подпись к фото: ' + caption if caption else 'Без подписи.'}"
