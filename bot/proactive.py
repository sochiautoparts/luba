"""Люба Proactive — topic starter + conversation summaries."""
...
async def _check_and_start_topic(chat_id):
    ...
    try:
        ...
        if sent:
            await db.add_group_message(chat_id=chat_id, user_id=config.BOT_ID, username=config.BOT_USERNAME.lstrip("@"), first_name="Люба", content=text, is_media=False, is_bot=True)
