"""Люба Main — starts OpenClaw gateway subprocess + aiogram bot."""
import asyncio, logging, os, signal, subprocess, sys, time
...
class LyubaBot:
    ...
    async def start(self):
        logger.info("=== Люба (OpenClaw) стартует ===")
        try:
        ...
        try:
            ...
            logger.info(f"Partners loaded: {len(partner_manager.campaigns)}")
        except: pass
        try:
    ...
    async def _notify_owner(self):
        ...
        try:
            await self.bot.send_message(config.OWNER_ID, f"Я на связи 😊 Люба, сейчас я {mood}. OpenClaw: {config.OPENCLAW_URL}. Провайдеры: {config.providers_status()}. Пиши или добавь в группу/канал 💬")
        except: pass
