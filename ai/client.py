...
async def _call_pollinations_direct(messages, max_tokens, timeout=30.0):
    if _client is None: await initialize()
    payload = {"model": _POLLINATIONS_MODEL, "messages": messages, "temperature": 0.9, "max_tokens": max_tokens, "stream": False, "referrer": "asluba-bot"}
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if choices:
                ...
                if content: return content if not _looks_garbage(content) else ""
...
_STATIC_FALLBACKS = {
    "greeting": ["Привет! Я Люба 😊", "Привет-привет! ☕", "Хей! Как настроение?"],
...
async def chat(prompt, system="", extra_context="", dialog_history=None, max_tokens=600, temperature=0.9, allow_static_fallback=True, fast=False, prefer_local=False):
    ...
    if fast:
        ...
        # Cloudflare FIRST (reliable, real content; tries both accounts)
...
async def _call_cloudflare(messages, max_tokens=600, timeout=60.0):
